    ...
    with tempfile.TemporaryDirectory(prefix='notryn-library-test-') as temp:
        ...
        try:
            with sync_playwright() as p:
                ...
                browser.close()
                print('PASS: current-folder creation, stale selection, empty folders, draft protection, save, drag up, back focus, context menus, shortcut preferences, read-only, responsive 320–1440px and demo reset.')
        finally:
            for server in [app, demo]:
                server.shutdown()
                server.server_close()
